// FinTrack Pro: Intelligence Edition (Insights + Search/Sort + Zero Hook Errors)
'use client';
...
const CustomChartTooltip = ({ active, payload, label, isPie = false }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-black/80 backdrop-blur-xl border border-white/20 p-4 rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200">
        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">{isPie ? payload[0].name : label}</p>
...
export default function Dashboard() {
  ...
  const processedData = useMemo(() => {
    ...
    if (searchQuery) {
      data = data.filter(t => t.category.toLowerCase().includes(searchQuery.toLowerCase()));
    }
      ...
      <div className="relative z-10 w-full px-4 sm:px-10 py-10 space-y-12 max-w-full mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center border-b border-white/5 pb-8 gap-6">
          ...
          <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 backdrop-blur-md">
            <button onClick={() => setRole('viewer')} className={`px-8 py-2.5 rounded-xl text-[10px] font-black transition-all ${role === 'viewer' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-gray-300'}`}>VIEWER</button>
        ...
        <section ref={chartRef} className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          <div className={`p-8 rounded-[40px] bg-white/[0.02] border border-white/5 h-[400px] backdrop-blur-md shadow-xl transition-all ${chartVisible ? 'reveal-active' : 'reveal-idle'}`}>
            <h2 className="text-[11px] font-black tracking-widest text-indigo-400 uppercase mb-10">Capital Flow</h2>
            <div className="h-64 w-full"><ResponsiveContainer><LineChart data={transactions}><Tooltip content={<CustomChartTooltip />}/> <Line type="monotone" dataKey="amount" stroke="#6366f1" strokeWidth={5} dot={false} /></LineChart></ResponsiveContainer></div>
          </div>
          <div className={`p-8 rounded-[40px] bg-white/[0.02] border border-white/5 h-[400px] flex items-center justify-center backdrop-blur-md shadow-xl transition-all ${chartVisible ? 'reveal-active' : 'reveal-idle'}`}>
             <ResponsiveContainer><PieChart><Pie data={expenseData} innerRadius={90} outerRadius={120} dataKey="value" paddingAngle={8} stroke="none">{expenseData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip content={<CustomChartTooltip isPie={true} />}/></PieChart></ResponsiveContainer>
          </div>
        </section>

        {/* ADMIN FORM */}
        {role === 'admin' && (
          <section ref={formRef} className={`p-10 rounded-[40px] border-2 transition-all duration-300 ${editingId ? 'bg-indigo-600/[0.03] border-indigo-500/30' : 'bg-white/[0.02] border-white/5'} ${formVisible ? 'reveal-active' : 'reveal-idle'} shadow-2xl`}>
            <div className="flex justify-between items-center mb-8">
               ...
               {editingId && <button onClick={() => {setEditingId(null); setForm({ date: '', amount: '', category: '', type: 'expense' });}} className="text-[10px] font-black text-red-500 tracking-widest">CANCEL</button>}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
              <input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})} className="bg-white/5 border border-white/10 rounded-2xl px-6 py-4 font-bold outline-none focus:border-indigo-500 transition-all text-sm text-white" />
              <input type="number" placeholder="Magnitude (₹)" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className="bg-white/5 border border-white/10 rounded-2xl px-6 py-4 font-bold outline-none focus:border-indigo-500 transition-all text-sm text-white" />
              <input type="text" placeholder="Classification" value={form.category} onChange={e => setForm({...form, category: e.target.value})} className="bg-white/5 border border-white/10 rounded-2xl px-6 py-4 font-bold outline-none focus:border-indigo-500 transition-all text-sm text-white" />
            ...
            <button onClick={handleSubmit} className="mt-8 bg-indigo-600 hover:bg-indigo-500 px-10 py-4 rounded-[18px] text-[11px] font-black tracking-widest shadow-lg transition-all uppercase text-white">{editingId ? 'Update Ledger' : 'Commit Entry'}</button>
          </section>
        )}
        ...
        <section ref={tableRef} className={`space-y-8 ${tableVisible ? 'reveal-active' : 'reveal-idle'}`}>
          <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-6">
            ...
            <div className="flex items-center gap-4 bg-white/5 px-6 py-3 rounded-[22px] border border-white/10 backdrop-blur-xl">
          ...
          <div className="rounded-[48px] bg-white/[0.01] border border-white/5 shadow-2xl backdrop-blur-3xl overflow-y-auto max-h-[600px] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-indigo-500/20 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-indigo-500/40">
            <table className="w-full text-left">
              <thead className="bg-white/5 text-[9px] font-black uppercase tracking-[0.3em] text-gray-500 border-b border-white/5"><tr className="px-8 py-6">
                <th className="px-8 py-6">Timestamp</th><th className="px-8 py-6">Classification</th><th className="px-8 py-6 text-right">Value</th>{role === 'admin' && <th className="px-8 py-6 text-center">Manage</th>}
              </tr></thead>
...
function Card({ title, value, color, icon }) {
  return (
    <div className="p-10 rounded-[48px] bg-white/[0.02] border border-white/5 flex justify-between items-center group hover:border-indigo-500/40 transition-all duration-700 shadow-xl relative overflow-hidden">
      <div className="space-y-2 relative z-10"><h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest group-hover:text-indigo-400">{title}</h3><p className={`text-3xl font-black tracking-tighter ${color}`}>₹{value.toLocaleString()}</p></div>
      <div className="relative z-10 p-5 bg-white/5 rounded-2xl text-gray-500 group-hover:text-indigo-400 group-hover:scale-110 transition-all">{icon}</div>
    </div>
  );
}